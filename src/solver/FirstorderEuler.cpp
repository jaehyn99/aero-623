...
void FirstorderEuler::buildFacesFromTriangularMesh() {
    ...
    // Resolve periodic titles by curve-id
    auto titleForCurveID = [&](int curveID) -> std::string {
        for (const auto& kv : boundaryFacesByTitle) {
            if (parseCurveId(kv.first) == curveID){
                return kv.first;
            }
        }
        return {};
    };
    const std::string c2 = titleForCurveID(2);
    const std::string c4 = titleForCurveID(4);
    const std::string c6 = titleForCurveID(6);
    const std::string c8 = titleForCurveID(8);
    if (!c2.empty() && !c4.empty()) {
        pairPeriodicTitles(c2, c4);
    }
    if (!c6.empty() && !c8.empty()) {
        pairPeriodicTitles(c6, c8);
    }

    std::unordered_map<std::string, std::size_t> groupFromTitle;
    std::vector<bool> visitedFace(static_cast<std::size_t>(triMesh_->numFaces()), false);
    ...
    for (std::size_t faceID = 0; faceID < static_cast<std::size_t>(triMesh_->numFaces()); ++faceID) {
        ...
        // Prevent the double count of the periodic edges
        if (visitedFace[faceID]){
            continue;
        }
        ...
        // const bool taggedPeriodic = (boundaryKindFromTitle(face._title, config_) == BoundaryKind::Periodic);
        if (periodic) {
            const int partner = periodicPartnerFace[faceID];
            visitedFace[faceID] = true;
            visitedFace[static_cast<std::size_t>(partner)] = true;
        }
        else {visitedFace[faceID] = true;
        }
        ...
        if (periodic) {
            ...
            localFaceR = findLocalFace(elemR, static_cast<int>(periodicFaceID));
        } else {
...
void FirstorderEuler::initUniformState() {
    ...
    const double T0 = p0 / (config_.rho0 * R);
...
std::vector<FirstorderEuler::EdgeFluxContribution> FirstorderEuler::computeEdgeFluxesAndWaveSpeeds() const {
    ...
    for (const auto& f : interiorFaces_) {
        ...
        const Eigen::Vector2d n(nFace[0], nFace[1]);
        ...
        edge.flux = F;
        edge.spectralRadius = std::max(spectralRadius(UL, nFace, config_.gamma),
                                    spectralRadius(UR, nFace, config_.gamma));
    ...
    for (const auto& f : boundaryFaces_) {
        ...
        edge.spectralRadius = spectralRadius(UL, nFace, config_.gamma);
...
void FirstorderEuler::advance(bool stopByTime) {
    ...
    while (iteration_ < config_.maxIterations && (!stopByTime || time_ < config_.finalTime)) {
        ...
        // VTK Writer Module.
        if (iteration_ % std::max<std::size_t>(1, config_.statusEvery) == 0 ||
            (stopByTime && time_ >= config_.finalTime)) {
