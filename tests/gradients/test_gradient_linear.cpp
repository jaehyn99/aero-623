...
#include "fv_FVGradient/FVGradient.h"
...
int main()
{
    ...
    std::shared_ptr<FVGradient> walkGrad = std::make_shared<WalkGrad>();
    states.setGradientMethod(walkGrad);
    GradVec grads = states.getFVGradient();

    // // ------------------ Compute FVGradient (Hybrid Walk Pn Grad) ------------------
    // std::shared_ptr<FVGradient> hybridWalkPNGrad = std::make_shared<HybridWalkPNGrad>();
